...
| 검수 후 라벨 | `Reviewed_Label` | **81.65%** | Epoch 10 | -5.43%p 저하 |
...
|------------|-----------|---------|------|
| 규칙 기반 라벨 | **0.0549** | 0.0313 | 상관관계 없음 (r ≤ 0.5) |

> 검수 후 라벨 모델은 학습 데이터 감소(160건 검수 반영 후 클래스 균등화로 3,756건) 및 클래스 재분포(중립 40→102건)의 영향으로 일부 클래스(부정·중립)에 대한 예측이 불안정하게 나타나, 해당 모델 기준의 Pearson Correlation 결과는 본 보고서에서 별도로 제시하지 않는다.
...
감성 클래스별 등락률 중간값 분석(`predict_market_correlation.py`)에서는 긍정 분류 날의 실제 상승, 부정 분류 날의 실제 하락이라는 방향성이 관찰되었다. 규칙 기반 라벨 기준 Pearson 상관계수는 r = **0.0549** (p = 0.0313)로, 통계적으로 유의미한 수준에 미치지 못한다.

검수 후 라벨의 경우, 직접 판단 검수로 인해 중립(2) 클래스가 40건에서 102건으로 크게 늘어나면서 클래스 분포가 재조정되었고, 균등 다운샘플링 과정에서 학습 데이터 규모도 일부 축소되었다. 그 결과 검증 정확도가 87.08%에서 **81.65%**로 저하되었으며, 일부 클래스에 대한 예측이 불안정하게 나타나 정량적 상관관계 비교에는 한계가 있었다.
